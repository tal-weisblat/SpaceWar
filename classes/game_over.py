...
class GameOver():
    ...
    def __init__(self):
        ...
        # # GAMEOVER 
        img_gameOver   = 'images/game_over.png'
        scale_gameOver = 0.2
        ...
        # PLAY AGAIN 
        img_plaAgain   = 'images/play_again.png'
        scale_playAgain = 1

        self.image = pg.image.load(img_plaAgain).convert_alpha()
        self.image_playAgain = pg.transform.scale(self.image,(scale_playAgain*int(self.image.get_width()),scale_playAgain*int(self.image.get_height())))  

        gameOver_height  = self.image_gameOver.get_height()
        gameOver_width  = self.image_gameOver.get_width()
        playAgain_width = self.image_playAgain.get_width()

        self.x_playAgain = SCREEN_WIDTH/2  - self.image_gameOver.get_width()/2 - (playAgain_width - gameOver_width)/2
        self.y_playAgain = SCREEN_HEIGHT/2 - self.image_gameOver.get_height()/2 + gameOver_height + 10 

        self.rect_playAgain = self.image_playAgain.get_rect()
        self.rect_playAgain.topleft = (self.x_playAgain,self.y_playAgain)      
        ...
        self.x_yes = self.x_playAgain + 20
        self.y_yes = self.y_playAgain + 100 
        ...
        self.x_no = self.x_yes + 100
        self.y_no = self.y_yes
    ...
    # self.rect.collidepoint(pos)             # mouse cursor hover above button 
    # pygame.mouse.get_pressed()[0] == 1      # mouse clicked 


    # NO - exit game 
    def no_button_clicked(self, pos, mouse_clicked):
    ...
    # YES - new game 
    def yes_button_clicked(self, pos, mouse_clicked):
